...
class DraftExpansion:
    ...
    def __init__(self, engine, batch_size: int):
        ...
        self.seen = torch.zeros(
            batch_size, model.num_moe_layers, model.num_experts,
            dtype=torch.bool, device=engine.device,
        )
        self.confidence = torch.ones(batch_size, device=engine.device)
        self.resident = torch.full_like(self.seen[0], engine.config.moe_backend == "fused")
        for layer, expert in engine.config.resident_experts:
